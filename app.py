...
LOGO_MUNICIPAL = Path("logo_traiguen.png")
LOGO_ICONO = Path("logo_icono.png")

st.set_page_config(
    page_title="Bodega Municipal de Traiguén",
    page_icon=str(LOGO_ICONO) if LOGO_ICONO.exists() else "📦",
    layout="wide",
)

# Logo institucional arriba a la izquierda, presente en toda la navegación.
if LOGO_MUNICIPAL.exists():
    try:
        st.logo(str(LOGO_MUNICIPAL), size="large")
    except Exception:
        # versiones antiguas de Streamlit no tienen st.logo
        st.sidebar.image(str(LOGO_MUNICIPAL), width=180)

# --------------------------------------------------------------- apariencia
# Paleta pedida por la municipalidad. El modo claro y el oscuro se resuelven
# con la preferencia del sistema del propio equipo, de modo que la persona ve
# el que ya tiene configurado sin tocar nada.
COLOR_CLARO_FONDO = "#7CF2D1"
COLOR_OSCURO_FONDO = "#121212"
...
st.markdown(
    ...
    <style>
      /* ---------------- MODO CLARO ---------------- */
      .stApp {{
          background-color: {COLOR_CLARO_FONDO};
          color: #000000;
      }}
      ...
      /* las tarjetas y tablas van en blanco para que el texto se lea */
      ...
      [data-testid="stSidebar"] {{
          background-color: #FFFFFF;
          border-right: 2px solid {AZUL_MUNICIPAL};
      }}
      ...
      .stTabs [aria-selected="true"] {{
          ...
          border-bottom-color: {AZUL_OSCURO} !important;
      }}
      ...
      @media (prefers-color-scheme: dark) {{
        .stApp {{
            background-color: {COLOR_OSCURO_FONDO};
            color: #FFFFFF;
        }}
        .stApp, .stApp p, .stApp li, .stApp label,
        .stApp h1, .stApp h2, .stApp h3, .stApp h4 {{
        ...
        /* marcos visibles: sin ellos, sobre gris carbón las tablas y
           tarjetas se confunden con el fondo */
        [data-testid="stDataFrame"], [data-testid="stTable"],
        [data-testid="stExpander"], [data-testid="stForm"],
        ...
        .stTabs [data-baseweb="tab-list"] {{ border-bottom: 1px solid #4A4A4A; }}
        .stTabs [aria-selected="true"] {{
            color: {AZUL_MUNICIPAL} !important;
      ...
      /* Aviso de pedido vacío: botón rojo que late, solo en esa situación */
      @keyframes latido {{
