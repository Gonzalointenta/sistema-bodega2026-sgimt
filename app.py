...
def panel_nueva_solicitud(es_encargado: bool, identidad: dict = None):
    ...
    if busqueda:
        ...
        candidatos = core.buscar_producto(busqueda, umbral=35)
        if candidatos:
            st.caption(
                ...
                ":red[**rojo**] poco confiable. **Puede marcar varios a la vez.**"
            )
            opciones = {
            ...
            elegidos = st.multiselect(
                "Marque los productos que necesita", list(opciones.keys()),
                key=f"multi_sel_{n}",
            )

            if elegidos:
                st.markdown("**Cantidad de cada uno**")
                cantidades = {}
                for etiqueta in elegidos:
                    codigo, nombre, _score = opciones[etiqueta]
                    c1, c2 = st.columns([4, 1])
                    c1.markdown(nombre)
                    cantidades[codigo] = c2.number_input(
                        f"cantidad {codigo}", min_value=1, step=1, value=1,
                        key=f"cant_{n}_{codigo}", label_visibility="collapsed",
                    )

                if st.button(f"Agregar {len(elegidos)} producto(s) a la solicitud",
                             type="primary"):
                    agregados = []
                    for etiqueta in elegidos:
                        codigo, nombre, score = opciones[etiqueta]
                        if score < 100 and es_encargado:
                            core.registrar_alias_nuevo(busqueda, codigo)
                        st.session_state.carrito.append(
                            (codigo, nombre, cantidades[codigo]))
                        agregados.append(f"{nombre} x{cantidades[codigo]}")
                    st.session_state.ultimo_agregado = " · ".join(agregados)
                    st.session_state.form_nonce += 1   # limpia búsqueda y cantidades
                    st.rerun()

            st.caption(MENSAJE_DERIVAR_ENCARGADO)
        else:
